// Ported from apps/cloud/app/download/page.tsx — fully static, no data
// fetching. The download button just links to GET /api/download/bridge
// (apps/api/src/routes/bridge.ts's `downloadRoutes`), which 302s to the CDN.
const STEPS = [
  ...
  return (
    <main className="mx-auto flex min-h-screen max-w-xl flex-col items-center justify-center gap-8 px-6 py-16">
      <div className="text-center">
        <h1 className="text-2xl font-semibold text-brand-green-dark">Mote Capture Bridge</h1>
      ...
      <div className="w-full rounded-xl border border-zinc-200 bg-white p-6">
        ...
        <p className="mt-4 text-xs text-muted-foreground">
          Cloud URL: <code>https://capture.motekreatif.com</code>. Bridge Token diambil dari menu
          Booth di dashboard admin (satu token per booth).
        </p>
      </div>
    </main>
  );
